  ...
  TrelloPowerUp.initialize({
        ...
        return Promise.all([Epic.isSubscription(t, cardId), Epic.getParent(t, cardId), isAuthorizedSafe(t)]).then(function (rr) {
          ...
          if (!authed) {
            buttons.push({
              ...
              callback: function (t) { return t.popup({ title: Epic.L('authorize_duck'), url: url('views/authorize.html?v=77'), height: 160 }); },
            });
          }
          return buttons;
        });
    ...
    'card-back-section': function (t) {
      return t.card('id').then(function (c) {
        return {
          title: 'Duck Epics',
          icon: ICON,
          content: { type: 'iframe', url: t.signUrl(url('views/section.html?v=77&c=' + c.id)), height: 200 },
        };
      });
    },
    ...
    'show-authorization': function (t) {
      return Epic.loadMessages(t).then(function () {
        return t.popup({ title: Epic.L('authorize_duck'), url: url('views/authorize.html?v=77'), height: 160 });
      });
    },
  }, {
    appKey: Epic.APP_KEY,
